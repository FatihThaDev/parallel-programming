...
template <typename T> void run() {
  ...
  if (!output_as_csv) {
    ...
    if (mibibytes) {
      // MiB = 2^20
      std::cout << std::setprecision(1) << std::fixed << "Array size: "
                << ARRAY_SIZE * sizeof(T) * std::pow(2.0, -20.0) << " MiB"
                << " (=" << ARRAY_SIZE * sizeof(T) * std::pow(2.0, -30.0)
                << " GiB)" << std::endl;
      std::cout << "Total size: "
                << 3.0 * ARRAY_SIZE * sizeof(T) * std::pow(2.0, -20.0) << " MiB"
                << " (=" << 3.0 * ARRAY_SIZE * sizeof(T) * std::pow(2.0, -30.0)
                << " GiB)" << std::endl;
    } else {
      // MB = 10^6
  ...
  auto initElapsedS =
      std::chrono::duration_cast<std::chrono::duration<double>>(read2 - read1)
          .count();
  auto readElapsedS =
      std::chrono::duration_cast<std::chrono::duration<double>>(init2 - init1)
          .count();
  auto initBWps = ((mibibytes ? std::pow(2.0, -20.0) : 1.0E-6) *
                   (3 * sizeof(T) * ARRAY_SIZE)) /
                  initElapsedS;
  auto readBWps = ((mibibytes ? std::pow(2.0, -20.0) : 1.0E-6) *
                   (3 * sizeof(T) * ARRAY_SIZE)) /
                  readElapsedS;

  if (output_as_csv) {
    std::cout << "phase" << csv_separator << "n_elements" << csv_separator
              << "sizeof" << csv_separator
              << ((mibibytes) ? "max_mibytes_per_sec" : "max_mbytes_per_sec")
              << csv_separator << "runtime" << std::endl;
    std::cout << "Init" << csv_separator << ARRAY_SIZE << csv_separator
              << sizeof(T) << csv_separator << initBWps << csv_separator
              << initElapsedS << std::endl;
    std::cout << "Read" << csv_separator << ARRAY_SIZE << csv_separator
              << sizeof(T) << csv_separator << readBWps << csv_separator
              << readElapsedS << std::endl;
  } else {
    std::cout << "Init: " << std::setw(7) << initElapsedS << " s (=" << initBWps
              << (mibibytes ? " MiBytes/sec" : " MBytes/sec") << ")"
              << std::endl;
    std::cout << "Read: " << std::setw(7) << readElapsedS << " s (=" << readBWps
              << (mibibytes ? " MiBytes/sec" : " MBytes/sec") << ")"
              << std::endl;
  }
  ...
  if (output_as_csv) {
    std::cout << "function" << csv_separator << "num_times" << csv_separator
              ...
              << ((mibibytes) ? "max_mibytes_per_sec" : "max_mbytes_per_sec")
              << csv_separator << "min_runtime" << csv_separator
              << "max_runtime" << csv_separator << "avg_runtime" << std::endl;
  } else {
    std::cout << std::left << std::setw(12) << "Function" << std::left
              ...
              << std::setw(12) << "Max" << std::left << std::setw(12)
              << "Average" << std::endl
              << std::fixed;
  }
  ...
  if (selection == Benchmark::All || selection == Benchmark::Nstream) {
    ...
    for (int i = 0; i < timings.size(); ++i) {
      ...
      if (output_as_csv) {
        std::cout << labels[i] << csv_separator << num_times << csv_separator
                  ...
                  << ((mibibytes) ? std::pow(2.0, -20.0) : 1.0E-6) * sizes[i] /
                         (*minmax.first)
    ...
    double bandwidth = ((mibibytes) ? std::pow(2.0, -30.0) : 1.0E-9) *
...
void check_solution(const unsigned int ntimes, std::vector<T> &a,
  ...
  long double errA =
      std::accumulate(a.begin(), a.end(), T{}, [&](double sum, const T val) {
        return sum + std::fabs(val - goldA);
      });
  errA /= a.size();
  long double errB =
      std::accumulate(b.begin(), b.end(), T{}, [&](double sum, const T val) {
        return sum + std::fabs(val - goldB);
      });
  errB /= b.size();
  long double errC =
      std::accumulate(c.begin(), c.end(), T{}, [&](double sum, const T val) {
        return sum + std::fabs(val - goldC);
      });
  errC /= c.size();
  ...
  if (errA > epsi)
    std::cerr << "Validation failed on a[]. Average error " << errA
              << std::endl;
  if (errB > epsi)
    std::cerr << "Validation failed on b[]. Average error " << errB
              << std::endl;
  if (errC > epsi)
    std::cerr << "Validation failed on c[]. Average error " << errC
              << std::endl;
  ...
  if (selection == Benchmark::All && errSum > 1.0E-8)
    std::cerr << "Validation failed on sum. Error " << errSum << std::endl
              << std::setprecision(15) << "Sum was " << sum << " but should be "
              << goldSum << std::endl;
}
...
void parseArguments(int argc, char *argv[]) {
  for (int i = 1; i < argc; i++) {
    if (!std::string("--list").compare(argv[i])) {
      ...
      std::cout << "      --csv                Output as csv table"
                << std::endl;
      std::cout << "      --mibibytes          Use MiB=2^20 for bandwidth "
                   "calculation (default MB=10^6)"
                << std::endl;
      std::cout << std::endl;
      exit(EXIT_SUCCESS);
    } else {
