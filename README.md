...
- Before running the next script, I had to run `scancel -u $USER`, because it was waiting for another job to finish.

- Then I ran `compute.sh`, without batch scheduler. I used this command: `salloc --ntasks=1 --cpus-per-task=2`, then I ran `./compute.sh`, and as you can see, all the resources are very much hogged by the script:
